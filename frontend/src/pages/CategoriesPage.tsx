  ...
  return (
    <section className="space-y-3">
      ...
      <table className="w-full border border-slate-300 bg-white text-[12px]">
        <thead className="bg-slate-100"><tr><th className="p-2 text-left">Code</th><th className="p-2 text-left">Name</th><th className="p-2 text-left">Status</th></tr></thead>
        <tbody>
          {(query.data ?? []).map((row) => (
            <tr key={row.ulid} className="border-t"><td className="p-2">{row.code}</td><td className="p-2">{row.name}</td><td className="p-2">{row.is_active ? 'Active' : 'Archived'}</td></tr>
          ))}
        </tbody>
      </table>
    </section>
